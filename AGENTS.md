...
- **Installable web app (PWA):** `web/manifest.json` + a minimal pass-through `web/sw.js` (installability only — deliberately NO cache strategy; the fetch handler forwards everything to the network) make the page installable as a home-screen app, which is what lets the installed app open **without a URL hash** and hit the stored-token auto-reconnect branch in `app.js` (a QR-opened page carries `#PIN` and deliberately clears the token instead — both paths are correct). Icons under `web/icons/` are derived from the same vendored Tabler microphone as the menu-bar icon (`web/vendor/tabler/`, MIT); regenerate them from that SVG if it ever changes. Everything under `web/` embeds automatically via rust-embed. `src/server/assets.rs` unit tests pin manifest validity, real icon pixel sizes (IHDR parse), and SW presence — keep them green when touching these files.
...
3. **Server-gone keeps the token and waits (auto-reconnect, no reload):** since T01 the TLS identity and session token **persist across restarts**, so when a server that went away comes back at the same address its certificate is the same one the page already accepted — the old "cert regenerated every start → reload required" premise is gone. On a confirmed "server gone" the client therefore **keeps the stored token**, shows the waiting notice (Reload button remains as an escape hatch only), and probes `/api/info` every ~3s (`waitForServerReturn`); once the server answers it renews the token and returns to the main screen without asking for the PIN. If the returned server rejects the token (e.g. another device paired meanwhile), it falls back to ordinary manual pairing. Each "server gone" verdict is confirmed first (a `503` is definitive; a network error or a single idle-check failure triggers one confirming `/api/info` probe).
